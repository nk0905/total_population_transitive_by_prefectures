...
// TODO 都道府県のデータ。APIが使用できるようになれば削除
const prefectures = [
  {
    prefCode: 1,
    prefName: '北海道',
...
const PoplationTransition: React.FC = () => {
  ...
  useEffect(() => {
    // TODO 「429. Too many requests.」が出たので一旦コメントアウト
    // getPrefectures().then((res) => {
    //   const newPrefecturesData: PrefecturesDataType[] = prefectures.map((prefecture) => {
  ...
  const handleChangePrefecturesCheckbox = (index: number) => {
    const editPrefectureData = deepCopy(prefecturesData);
    editPrefectureData[index].isChecked = !editPrefectureData[index].isChecked;
    setPrefecturesData(editPrefectureData);
  };
